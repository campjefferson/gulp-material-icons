...
module.exports = function(opts) {
  ...
  items.forEach(function(item) {
    Object.keys(item.icons).forEach(function(key) {
      var prefixLocal = require('path').resolve(__dirname, 'overrides/' + key);
      var prefixGlobal =
        require('path').dirname(require.resolve('material-design-icons')) +
        '/' +
        key +
        '/svg/production/';

      if (!require('fs').existsSync(require('path').resolve(prefixGlobal))) {
        throw new gutil.PluginError({
          plugin: PLUGIN_NAME,
      ...
      icons.forEach(function(name) {
        item.sizes.forEach(function(size) {
          var src = prefixLocal + '/ic_' + name + '_' + size + 'px.svg';
          src = require('path').relative(process.cwd(), src);
          if (!require('fs').existsSync(src)) {
            src = prefixGlobal + 'ic_' + name + '_' + size + 'px.svg';
            src = require('path').relative(process.cwd(), src);
          ...
          if (!srcs[src]) {
            ...
            srcs[src].tasks = srcs[src].tasks.concat(
              getTasks(item, name, size)
            );
          }
        });
      });
